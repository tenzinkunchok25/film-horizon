...
fetchDataFromServer(`https://api.themoviedb.org/3/movie/${movieId}?api_key=${api_key}&append_to_response=casts,videos,images,releases`, function (movie) {
    ...
    document.title = `${title} - FILM HORIZON`;
    ...
    //for (const { key, name } of filterVideos(videos)) {
        ...
        videoCard.innerHTML = `
        <iframe width="500" height="294" src="https://vidsrc-embed.ru/embed/${movieId}?&theme=dark&color=white&rel=0" frameborder="0" allowfullscreen="1" title="${name}" class="img-cover" loading="lazy"></iframe>`;
